...
class LaneDetection:
    ...
    def Angle(self, left_lane, right_lane, width, height):
        ...
        print("tan = " + str(float(float(x0 - width / 2) / float(y0))))
        theta = float(math.atan(float(float(x0 - width / 2) / float(y0))))
        print("theta = " + str(theta * 50))
        return theta * 30
    ...
    def run(self):
        ...
        while True:
            ...
            lines = cv2.HoughLinesP(frame_edge, rho=1, theta=np.pi / 180, threshold=50, minLineLength=10,
                                        maxLineGap=10)
            left_lanes = []
            right_lanes = []
